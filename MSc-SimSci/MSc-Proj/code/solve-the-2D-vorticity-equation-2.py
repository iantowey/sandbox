...
def get_particle_states0(u,v,x0,y0,dt):
    u0=interp2(u,x0,y0)
    v0=interp2(v,x0,y0)        
    x0_new=x0+u0*dt
    y0_new=y0+v0*dt

def get_particle_states1(s,d,x0,y0):       
    
    phi=np.matrix(np.zeros((Nx,Ny)))        
    ...
    psi_angle=(pi/4)-np.transpose(phi)

    psi_angle_hat=np.fft.fftshift(np.fft.fft(psi_angle)
    psi_angle_x=np.real(np.fft.ifft(np.fft.ifftshift(kx*psi_angle_hat))
    psi_angle_y=np.real(np.fft.ifft(np.fft.ifftshift(ky*psi_angle_hat))
    ...
    s=np.transpose(s)
    d=np.transpose(d)
    lambda_unsigned=sign(d)*sqrt(np.square(d)+np.square(s))
